...
public:
    ...
    void display(node *n, int counter); // We cannot give default arguments in here if fucntion is not defined inside the body
};
...
node *node::create(node *n, int counter = 0)
{
    ...
    if (flag == true)        // Creation of Root Node
    {
        ...
        for (int a = 0; a < rootnode->child_count; a++)
        {
            ...
            {
                create(&(chapter[a])); // Refrence Return an Address which is accetable to function with pointer parameter
            }
            else
            {
                continue;
